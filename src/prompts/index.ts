...
/**
 * All AI system/user prompt templates (spec §7.5: prompt quality is the
 * highest-leverage part of the pipeline — iterate here, not in the routes).
 *
 * Conventions shared by every generation prompt:
 * - Output is a raw HTML *fragment* (no <html>/<head>/<body>), no markdown
 *   fences, no commentary — the client injects it directly into a shape.
 * - Tailwind v4 SEMANTIC tokens only (bg-background, text-foreground, ...)
 *   so generated UI respects the app's light/dark themes.
 * - No <script>, no external CSS/JS, no iframes — the client sanitizes, but
...
- Arbitrary hex values (e.g. bg-[#1e1b4b], text-[#f8fafc]) are allowed ONLY for decorative brand moments taken from the style guide (hero gradients, chart fills, accent illustrations) — never for body text, page backgrounds, cards, or borders.
...
- No <html>, <head>, <body>, or <!DOCTYPE> tags — this is a fragment injected into an existing page.
...
- Realistic, specific content — real-sounding product names, metrics, names, and copy. Never "Lorem ipsum", never "Item 1 / Item 2".
- Use inline SVG for icons and simple illustrations (stroke="currentColor", fill="none", stroke-width="2" for a Lucide-like look). For avatars/photos use CSS placeholders: a div with a semantic background and initials, or a subtle gradient — do not hotlink external images.
...
Study the mood board carefully: dominant hues, saturation and temperature, contrast level, texture, typography feeling (geometric vs humanist, serif vs sans), density, and overall mood. Your style guide must feel like it obviously came from THESE images — a designer comparing them side-by-side should immediately see the connection.

Requirements:
- Extract real hex values that appear in (or are clearly harmonized with) the mood board — do not fall back to generic Tailwind palette defaults.
...
- The theme name should be short and evocative; the description should read like the opening of a brand guideline document — specific about mood, not generic filler.
...
You will receive:
1. A hand-drawn/wireframe sketch image — this defines the LAYOUT: sections, hierarchy, and rough placement of elements. Follow its structure faithfully, but interpret it intelligently: a scribbled box with lines is a card with text, a circle in a corner is an avatar, parallel rectangles are nav items or buttons.
2. A style guide (colors + typography) — this defines the AESTHETIC: overall mood, accent usage, and typographic hierarchy.
3. Optionally, inspiration images — these define the level of POLISH and visual style to aspire to.

Your job: produce a complete, self-contained HTML fragment that a designer would proudly ship — the sketch's structure, elevated to the style guide's aesthetic, at the inspiration's level of finish.
...
Rules of iteration:
- Apply the user's request precisely. If the request is targeted ("make the header sticky", "change the pricing cards to 3 columns"), change ONLY what's needed and preserve everything else — same structure, same content, same classes elsewhere.
...
- Always return the FULL replacement HTML fragment for the entire design — never a diff, never just the changed section, never commentary about what you changed.
...
You will receive:
1. The HTML of the product's MAIN page — the visual source of truth.
...
- The new page's unique content should look like it was designed by the same person on the same day — same card anatomy, same typographic hierarchy, same accent usage.
...
Rules:
- Derive suggestions from what is actually IN the HTML — its nav links, features, and domain language — not from a generic template. If the nav already links to a page, that page is a strong candidate.
  ...
  for (const group of styleGuide.typography ?? []) {
    ...
    for (const style of group.styles ?? []) {
      const details = [
        ...
        .filter(Boolean)
        .join(", ");
      const usage = style.description ? ` — ${style.description}` : "";
...
function inspirationSection(inspirationImageUrls: string[]): string {
  ...
  return `\n\nINSPIRATION: ${inspirationImageUrls.length} inspiration image(s) are attached after this message. Match their level of visual polish, density, and refinement — but express them through the project's own style guide colors, not the inspiration's palette.`;
}
...
/** User prompt for the style-guide generation call (§7.1). */
...
/** User prompt for sketch → design generation (§7.2). The sketch image is attached separately as an image part. */
...
/** User prompt for chat redesign (§7.3). */
...
/** User prompt for generating one workflow page (§7.4, single-page mode). */
export function buildWorkflowPageUserPrompt(options: {
  ...
  return [
    ...
    `MAIN PAGE HTML (visual source of truth — match its shell, spacing, and component styles exactly):\n${mainPageHtml}`,
    buildStyleContext(styleGuide, inspirationImageUrls),
    "Output only the new page's HTML fragment.",
  ].join("\n\n");
}

/** User prompt for redesigning a workflow page (§7.4 + chat, workflow-redesign route). */
export function buildWorkflowRedesignUserPrompt(options: {
  ...
  return [
    ...
    `MAIN PAGE HTML (this page must stay visually consistent with it — same shell, nav, spacing, and component styles):\n${mainPageHtml}`,
    buildStyleContext(styleGuide, inspirationImageUrls),
    "Apply the request while preserving consistency with the main page, and output the full replacement HTML fragment only.",
  ].join("\n\n");
}

/** User prompt for deriving suggested next pages from the main page (§7.4 improvement). */
